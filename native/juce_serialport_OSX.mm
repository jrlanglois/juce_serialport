...
StringPairArray SerialPort::getSerialPortPaths()
{
    ...
    if (IOServiceGetMatchingServices(masterPort, classesToMatch, &matchingServices) != KERN_SUCCESS)
    {
...
bool SerialPort::open(const String& portPath)
{
    ...
    // non canonical, 0.5 second timeout, read returns as soon as any data is recieved
    cfmakeraw(&options);
    options.c_cc[VMIN] = 0;
...
bool SerialPort::setConfig(const SerialPortConfig &config)
{
    ...
    // baud and bits
    // Just set 9600 to get this to pass, we'll set the actual baud rate later
    ...
    auto new_baud = static_cast<int>(config.bps);
...
bool SerialPort::getConfig(SerialPortConfig &config)
{
    ...
    config.bps = ((int)cfgetispeed(&options)) > ((int)cfgetospeed(&options))
...
void SerialPortOutputStream::run()
{
    ...
    while (port && (port->portDescriptor != -1) && !threadShouldExit())
    {
        ...
        if (bufferedbytes)
        {
            ...
            int bytestowrite = bufferedbytes > writeBufferSize
                                ? writeBufferSize
                                : bufferedbytes;
